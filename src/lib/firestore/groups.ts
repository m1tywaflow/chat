...
// Firestore не принимает undefined, поэтому размеры добавляем в документ только если они есть.
...
export async function sendGroupMessage(
  ...
  await runTransaction(db, async (transaction) => {
    ...
    transaction.set(msgRef, {
      text,
      imageUrl: imageUrl ?? null,
      ...imageSizeFields(imageUrl, imageWidth, imageHeight),
      senderId,
      senderName,
      senderAvatarUrl: senderAvatarUrl ?? null,
      replyTo: replyTo ?? null,
...
export async function leaveGroup(groupId: string, uid: string): Promise<void> {
  ...
  await runTransaction(db, async (transaction) => {
    ...
    if (data.ownerId === uid) {
      throw new Error(
        "The group owner must delete the group instead of leaving"
      );
    }
