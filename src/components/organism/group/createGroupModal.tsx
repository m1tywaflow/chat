...
//               >
//                 <div
//                   className={`absolute -inset-[3px] rounded-full transition-opacity duration-300 ${isDraggingAvatar ? "cgm-ring opacity-100" : "opacity-0"
//                     }`}
//                 />
...
//                 ) : (
//                   <div
//                     className={`absolute inset-0 rounded-full border border-dashed transition-colors ${isDraggingAvatar
...
//                     className={`cgm-row flex items-center gap-2.5 px-1.5 py-2 rounded-xl transition-colors cursor-pointer text-left ${isSelected
//                         ? "bg-[#7c5cff]/[0.08]"
//                         : "hover:bg-white/[0.04]"
//                       }`}
//                   >
...
//                     </span>
//                     <div
//                       className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 transition-all duration-150 ${isSelected
  ...
  return (
    ...
    >
      <style>{`
        ...
        .cgm-scroll::-webkit-scrollbar { width: 4px; }
        .cgm-scroll::-webkit-scrollbar-track { background: transparent; }
        .cgm-scroll::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.09); border-radius: 999px; }
        .cgm-scroll::-webkit-scrollbar-thumb:hover { background: rgba(255,255,255,0.18); }
        .cgm-chips::-webkit-scrollbar { display: none; }
        .cgm-row { animation: rowIn 0.15s ease both; }
        .cgm-chip { animation: chipIn 0.15s cubic-bezier(0.34,1.4,0.64,1) both; }
        .cgm-error { animation: errorIn 0.15s ease both; }
        .cgm-ring { background: conic-gradient(from 0deg, #7c5cff, #a78bfa, #2b1f78, #7c5cff); animation: ringSpin 3s linear infinite; }
      `}</style>
      ...
      <div
        ...
        onClick={(e) => e.stopPropagation()}
      >
        <div className="pointer-events-none absolute -top-32 left-1/2 -translate-x-1/2 w-80 h-80 rounded-full bg-[#7c5cff]/[0.14] blur-[110px]" />
        <div className="pointer-events-none absolute -bottom-24 -right-16 w-56 h-56 rounded-full bg-[#a78bfa]/[0.08] blur-[100px]" />

        <div className="relative rounded-2xl border border-white/[0.07] bg-[#07060d]/95 backdrop-blur-xl shadow-[0_24px_80px_-16px_rgba(0,0,0,0.65)] flex flex-col">
          {/* Header */}
          <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.06] bg-white/[0.015]">
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 rounded-lg bg-[#7c5cff]/[0.12] flex items-center justify-center">
                <Users size={13} className="text-[#a996ff]" />
              </div>
              <h3 className="text-[14.5px] font-semibold text-white tracking-[-0.01em]">
                New group
              </h3>
              {selected.size > 0 && (
                <span className="text-[10.5px] font-semibold text-[#a996ff] bg-[#7c5cff]/[0.14] border border-[#7c5cff]/[0.18] px-2 py-[3px] rounded-md tabular-nums">
                  {selected.size} {pluralizeMembers(selected.size)}
                </span>
              )}
            </div>
            <button
              onClick={onClose}
              disabled={creating}
              className="w-7 h-7 flex items-center justify-center rounded-lg text-zinc-500 hover:text-white hover:bg-white/[0.07] active:scale-90 transition-all cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed"
            >
              <X size={14} />
            </button>
          </div>
          ...
          <div className="px-5 pt-5 pb-5 flex flex-col gap-4">
            ...
            <div className="flex items-center gap-3.5">
              ...
              <div
                ...
                onDragLeave={() => setIsDraggingAvatar(false)}
                onDrop={handleAvatarDrop}
                className="group relative shrink-0 w-16 h-16 rounded-full cursor-pointer"
              >
                <div
                  className={`absolute -inset-[3px] rounded-full transition-opacity duration-300 ${isDraggingAvatar ? "cgm-ring opacity-100" : "opacity-0"
                    }`}
                ...
                {avatarPreview && (
                  <button
                    onClick={handleRemoveAvatar}
                    className="absolute -bottom-1 -right-1 w-5 h-5 rounded-md bg-[#1a1826] border border-white/10 flex items-center justify-center text-zinc-400 hover:text-red-400 hover:border-red-400/30 transition-colors cursor-pointer shadow-md"
                  >
                    <Trash2 size={10} />
                  </button>
                )}
              ...
              <div className="flex-1 min-w-0 relative">
                <input
                  ...
                  onKeyDown={handleNameKeyDown}
                  placeholder="Group name"
                  className="w-full bg-white/[0.04] border border-white/[0.07] rounded-lg px-3.5 h-11 text-[14px] font-medium text-white placeholder-white/25 outline-none focus:border-[#7c5cff]/50 focus:bg-white/[0.05] transition-colors"
                />
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-white/20 tabular-nums pointer-events-none">
                  {nameLen}/40
                </span>
              </div>
            </div>
            ...
            {/* Search */}
            <div className="flex items-center gap-2.5 px-3.5 h-10 rounded-lg bg-white/[0.04] border border-white/[0.06] focus-within:border-[#7c5cff]/40 focus-within:bg-white/[0.05] transition-colors">
              <Search size={14} className="text-white/25 shrink-0" />
              <input
                value={search}
                onChange={(e) => setSearch(e.target.value)}
              ...
              {filteredContacts.length > 0 && (
                <button
                  onClick={toggleSelectAllFiltered}
                  className="shrink-0 text-[11px] font-semibold text-[#a996ff] hover:text-white transition-colors cursor-pointer whitespace-nowrap"
                >
                  {allFilteredSelected ? "Deselect all" : "Select all"}
                </button>
              )}
            ...
            {/* Selected chips — reserve space so the list below never jumps */}
                ...
                <div className="cgm-chips flex items-center gap-1.5 -mx-1 px-1 overflow-x-auto">
                  {selectedContacts.map((c) => (
                    <button
                      key={c.uid}
                      onClick={() => toggleContact(c.uid)}
                      className="cgm-chip group shrink-0 flex items-center gap-1.5 pl-1 pr-2 py-1 rounded-lg bg-[#7c5cff]/[0.1] border border-[#7c5cff]/[0.16] hover:bg-[#7c5cff]/[0.16] transition-colors cursor-pointer"
                    >
                      <div className="w-5 h-5 rounded-full overflow-hidden bg-white/10 shrink-0 flex items-center justify-center">
                        {c.avatarUrl ? (
                          <img
                      ...
                      <span className="text-[11.5px] text-white/80 max-w-[80px] truncate">
                        {c.username}
                      </span>
                      <X
                        size={10}
            ...
            {/* Contact list — fixed height so filtering never shifts the layout */}
            <div className="cgm-scroll h-56 overflow-y-auto -mx-1.5 flex flex-col gap-0.5">
              {filteredContacts.length === 0 && (
              ...
              {filteredContacts.map((c, i) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => toggleContact(c.uid)}
                    style={{ animationDelay: `${Math.min(i, 8) * 12}ms` }}
                    className={`cgm-row flex items-center gap-2.5 px-2 py-2 rounded-lg transition-colors cursor-pointer text-left ${isSelected
                      ? "bg-[#7c5cff]/[0.1] border border-[#7c5cff]/[0.14]"
            ...
            {error && (
              <div className="cgm-error flex items-center gap-2 px-3 py-2 rounded-lg bg-red-500/[0.08] border border-red-500/20">
                <AlertCircle size={13} className="text-red-400 shrink-0" />
                <p className="text-[11.5px] text-red-300">{error}</p>
              </div>
            )}
            ...
            <button
              onClick={handleCreate}
              disabled={!canCreate}
              className="w-full h-11 rounded-lg text-[13.5px] font-semibold text-white transition-all disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer bg-gradient-to-r from-[#7c5cff] to-[#5b3df0] hover:from-[#8d70ff] hover:to-[#6c4dff] active:scale-[0.98] shadow-[0_8px_24px_-8px_rgba(124,92,255,0.55)] flex items-center justify-center gap-2"
            >
              {creating ? (
                <>
                  <span className="w-3.5 h-3.5 border-2 border-white/40 border-t-white rounded-full animate-spin" />
