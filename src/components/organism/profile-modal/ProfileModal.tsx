...
//                   {avatarUploading ? (
//                     <div className="w-full h-full flex items-center justify-center bg-[#0f1520]">
...
async function uploadToCloudinary(file: Blob | File, folder: string): Promise<string> {
  const formData = new FormData();
  formData.append("file", file);
  formData.append("upload_preset", "jhravxtb");
  formData.append("folder", folder);
  const res = await fetch("https://api.cloudinary.com/v1_1/dgylh67ms/image/upload", { method: "POST", body: formData });
  if (!res.ok) throw new Error("Upload failed");
  const data = await res.json();
  return data.secure_url;
}
...
export default function ProfileModal({ onClose, userId }: ProfileModalProps) {
  ...
  useEffect(() => {
    ...
    if (!user) return;
    const date = new Date(user.metadata.creationTime!);
    setJoined(date.toLocaleDateString("en-US", { month: "long", year: "numeric" }));
  }, [targetUid]);
  ...
  function handleBannerFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (file.type === "image/gif") {
      ...
      setBannerUploading(true);
      uploadToCloudinary(file, "banners")
        .then((url) => { setDraftBanner(url); setDraftBannerIsImage(true); })
        .catch((err) => { console.error("Banner upload failed:", err); setBannerLocalPreview(null); })
        .finally(() => setBannerUploading(false));
      return;
    }
    setCropSrc(URL.createObjectURL(file));
  ...
  function handleAvatarFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (file.type === "image/gif") {
      ...
      setAvatarUploading(true);
      uploadToCloudinary(file, "avatars")
        .then((url) => { setDraftAvatar(url); })
        .catch((err) => { console.error("Avatar upload failed:", err); setAvatarLocalPreview(null); })
        .finally(() => setAvatarUploading(false));
      return;
    }
    setCropSrc(URL.createObjectURL(file));
  ...
  const activeBannerValue = editing ? bannerLocalPreview || draftBanner : bannerGradient;
  const activeBannerIsImage = editing ? (!!bannerLocalPreview || draftBannerIsImage) : bannerIsImage;
  const activeBorder = editing ? draftBorder : avatarBorder;
  const activeAvatar = editing ? (avatarLocalPreview || draftAvatar || avatar) : avatar;
  const activeCardColor = editing ? draftCardColor : cardColor;

  const bannerStyle = activeBannerIsImage
    ? { backgroundImage: `url(${activeBannerValue})`, backgroundSize: "cover", backgroundPosition: "center" }
    : { background: activeBannerValue };

  return (
    <>
      {cropSrc && (
        <ImageCropper src={cropSrc} aspectRatio={cropTarget === "banner" ? 320 / 88 : 1} onConfirm={handleCropConfirm} onCancel={handleCropCancel} label={cropTarget === "banner" ? "Adjust banner" : "Adjust avatar"} />
      )}

      <div className="fixed inset-0 bg-black/55 flex items-center justify-center z-50" onClick={onClose}>
        <div className="border border-white/[0.08] rounded-2xl w-[320px] flex flex-col items-center relative overflow-hidden transition-colors duration-200" style={{ backgroundColor: activeCardColor }} onClick={(e) => e.stopPropagation()}>

          <div className="w-full h-[88px] relative z-0 shrink-0" style={bannerStyle}>
            {bannerUploading && (
              <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
              </div>
            ...
            {editing && (
              <button onClick={() => bannerInputRef.current?.click()} className="absolute inset-0 flex items-center justify-center bg-black/30 hover:bg-black/50 transition-colors group">
                <div className="flex items-center gap-1.5 text-white/50 group-hover:text-white text-[11px] tracking-widest transition-colors">
                  <Upload size={13} />
                  {bannerUploading ? "UPLOADING..." : "CHANGE BANNER"}
                </div>
            ...
            <button onClick={onClose} className="absolute top-3 right-3 text-white/40 hover:text-white/80 transition-colors bg-black/20 rounded-full p-0.5">
              <X size={18} />
            </button>
          </div>
          ...
          <div className="w-full flex flex-col items-center px-8 pb-7 relative">
            <div className="relative z-10 -mt-9 mb-3 shrink-0">
              <div className="w-[72px] h-[72px] rounded-full p-[2.5px]" style={{ background: activeBorder }}>
                <div className="w-full h-full rounded-full flex items-center justify-center overflow-hidden" style={{ backgroundColor: activeCardColor }}>
                  {avatarUploading ? (
                    <div className="w-full h-full flex items-center justify-center">
                      <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                    </div>
                  ) : activeAvatar ? (
                    <img src={activeAvatar} alt="avatar" className="w-full h-full rounded-full object-cover" />
                  ) : (
                    <span className="text-white text-lg font-medium">{username?.[0]?.toUpperCase()}</span>
                  )}
                </div>
              </div>
              {editing && (
                <button onClick={() => avatarInputRef.current?.click()} className="absolute inset-0 rounded-full flex items-center justify-center bg-black/40 hover:bg-black/60 transition-colors">
                  <Upload size={14} className="text-white/60 hover:text-white" />
                </button>
              )}
            </div>
            ...
            <input ref={colorInputRef} type="color" className="hidden" value={draftCardColor} onChange={(e) => setDraftCardColor(e.target.value)} />

            <div className="flex flex-col items-center gap-1 mb-4">
              <span className="text-white font-semibold text-lg tracking-[0.03em]">{username}</span>
              <span className="text-[10px] text-white/22 tracking-widest uppercase">joined {joined}</span>
              <div className="mt-1 px-3 py-1 flex items-center gap-2 bg-gradient-to-r from-[#A78BFA]/20 to-[#60A5FA]/20 border border-white/10 text-xs text-white rounded-lg">
                Early Member
                <Gem size={14} color="gold" />
              </div>
            ...
            {bio && !editing && (
              <p className="text-xs text-white/40 text-center leading-relaxed mb-4">{bio}</p>
            )}

            {editing ? (
              <div className="w-full flex flex-col gap-5 mb-5">
                <div>
                  <p className="text-[10px] text-white/35 uppercase tracking-[0.2em] mb-2">Banner</p>
                  <div className="grid grid-cols-4 gap-2">
                    {BANNER_PRESETS.map((p) => (
                      <button key={p.id} onClick={() => { setDraftBanner(p.value); setDraftBannerIsImage(false); setBannerLocalPreview(null); }} className="h-8 rounded-lg transition-all" style={{ background: p.value, outline: !draftBannerIsImage && !bannerLocalPreview && draftBanner === p.value ? "2px solid #A78BFA" : "2px solid transparent", outlineOffset: "2px" }} />
                    ))}
                  </div>
                </div>

                <div>
                  <p className="text-[10px] text-white/35 uppercase tracking-[0.2em] mb-2">Avatar border</p>
                  <div className="grid grid-cols-6 gap-2">
                    {AVATAR_BORDERS.map((b) => (
                      <button key={b.id} onClick={() => setDraftBorder(b.value)} className="h-7 rounded-full transition-all" style={{ background: b.value, outline: draftBorder === b.value ? "2px solid #A78BFA" : "2px solid transparent", outlineOffset: "2px" }} />
                    ))}
                  </div>
                </div>

                <div>
                  <p className="text-[10px] text-white/35 uppercase tracking-[0.2em] mb-2">Card color</p>
                  <div className="flex items-center gap-2">
                    <div className="grid grid-cols-8 gap-1.5 flex-1">
                      {CARD_COLOR_PRESETS.map((c) => (
                        <button key={c} onClick={() => setDraftCardColor(c)} className="h-6 rounded-md transition-all" style={{ backgroundColor: c, border: draftCardColor === c ? "2px solid #A78BFA" : "2px solid rgba(255,255,255,0.1)" }} />
                      ))}
                    </div>
                    <button onClick={() => colorInputRef.current?.click()} title="Custom color" className="shrink-0 w-8 h-8 flex items-center justify-center rounded-lg border border-white/[0.10] hover:border-[#A78BFA]/50 text-white/40 hover:text-[#A78BFA] transition-colors relative overflow-hidden" style={{ backgroundColor: draftCardColor }}>
                ...
                <div className="flex gap-2">
                  <button onClick={() => { setEditing(false); setBannerLocalPreview(null); setAvatarLocalPreview(null); setDraftAvatar(null); }} className="flex-1 py-2 text-[11px] text-white/30 hover:text-white/60 border border-white/[0.08] rounded-lg transition-colors tracking-widest">CANCEL</button>
                  <button onClick={saveEdit} disabled={saving || bannerUploading || avatarUploading} className="flex-1 py-2 text-[11px] text-[#A78BFA] hover:text-white border border-[#A78BFA]/30 hover:border-[#A78BFA]/60 rounded-lg transition-colors tracking-widest flex items-center justify-center gap-1.5 disabled:opacity-40">
              ...
              <>
                {gifts.length > 0 && (
                  <div className="w-full mb-4">
                    <div className="text-[10px] text-white/35 uppercase tracking-[0.2em] text-center mb-3">Gifts</div>
                    <div className="grid grid-cols-2 gap-4">
                      {gifts.map((giftId, i) => {
                        const gift = GIFTS[giftId];
                        if (!gift) return null;
                        return (
                          <div key={giftId + i} className="flex flex-col items-center gap-1.5">
                            <div className="w-full aspect-square rounded-2xl flex items-center justify-center" style={{ background: `${RARITY_COLORS[gift.rarity]}15`, border: `1px solid ${RARITY_COLORS[gift.rarity]}40` }}>
                              <img src={gift.imageUrl} alt={gift.name} className="w-4/5 h-4/5 object-contain" />
                            </div>
                            <span className="text-[10px]" style={{ color: RARITY_COLORS[gift.rarity] }}>{gift.name}</span>
                          </div>
                        );
                      })}
                    </div>
                ...
                <div className="flex items-center justify-between w-full">
                  {isOwnProfile ? (
                    <button onClick={logout} className="text-[11px] text-red-400/55 hover:text-red-400 tracking-widest transition-colors">LOG OUT</button>
                  ) : <div />}
                  {isOwnProfile && (
                    <button onClick={openEdit} className="flex items-center gap-1.5 text-[11px] text-white/30 hover:text-white/70 tracking-widest transition-colors">
                      <Pencil size={12} />
                      EDIT
                    </button>
                  )}
