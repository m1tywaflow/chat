...
export default function ChatWindow() {
  ...
  async function loadOlder() {
    ...
    try {
      const { messages: older, hasMore } = await loadOlderMessages(
        chatId,
        oldest
      );
      if (activeChatIdRef.current !== chatId) return;
  ...
  // Older messages are prepended above the current scroll position, which
  // would otherwise yank the view down/up. Restore the exact offset once
  // the DOM has grown with the new content.
  useLayoutEffect(() => {
    const el = chatScrollRef.current;
  ...
  const canSend = (text.trim() || imageFile) && !uploading;
  // A pending (optimistic) bubble and its confirmed Firestore counterpart can
  // briefly coexist: the snapshot listener often delivers the real message
  // before the sendMessage() promise even resolves, so waiting for that
  // promise to clear pendingMessages left a visible flash of a duplicate
  // that then collapsed into one. Instead, hide a pending message the moment
  // a matching confirmed message shows up, matching each confirmed message
  // to at most one pending bubble (oldest first) so genuine repeated sends
  // of identical text aren't merged into a single bubble.
  const consumedConfirmedIdx = new Set<number>();
  const visiblePendingMessages = pendingMessages.filter((p) => {
    const idx = messages.findIndex(
      (m, i) =>
        !consumedConfirmedIdx.has(i) &&
        m.senderId === p.senderId &&
        (m.text || "") === (p.text || "") &&
        (m.imageUrl || null) === (p.imageUrl || null)
    );
    if (idx === -1) return true;
    consumedConfirmedIdx.add(idx);
    return false;
  });
  const displayMessages = [...messages, ...visiblePendingMessages];
  const currentMsgMenu = msgMenu
    ? displayMessages.find((m) => m.id === msgMenu.id)
    : null;
