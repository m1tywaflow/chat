...
//       setIsFileVideo(false);
//       setTyping(chatId, myUid, false);
//       // sending a message should always snap the view back to the bottom,
//       // exactly like Telegram, regardless of where you were scrolled to
//       isNearBottom.current = true;
...
function StatusTick({
  ...
  if (pending) return <Clock size={size - 2} className="opacity-60" />;
  return isRead ? <CheckCheck size={size} /> : <Check size={size} />;
}
      ...
      >
        ...
        >
          {displayMessages.map((m) => {
            ...
            const isLastMine =
              isMine &&
              displayMessages.filter((msg) => msg.senderId === myUid).at(-1)
                ?.id === m.id;
            const reactionSummary = getReactionSummary(m.reactions);
            const hasReactions = reactionSummary.length > 0;
            const isPickerOpen = pickerOpenId === m.id;
                      ...
                      {m.deleted ? (
                        ...
                        <>
                          ...
                          {!m.text && m.imageUrl && isMine && isLastMine && (
                            ...
                            >
                              <StatusTick pending={m.pending} isRead={isRead} />
                            </div>
                          )}
                        </>
                      )}
