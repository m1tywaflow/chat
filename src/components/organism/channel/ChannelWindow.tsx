...
async function uploadPostImage(file: File) {
  ...
  return { url: data.secure_url as string, width: data.width as number, height: data.height as number };
}
...
const tinySrc = (url: string) => cloudinaryUrl(url, "e_blur:1000,q_1,w_24,f_auto"); // ~1-2кб
const fullSrc = (url: string, w = 760) => cloudinaryUrl(url, `f_auto,q_auto,dpr_auto,w_${w}`);
...
export default function ChannelWindow({
  ...
  async function handlePost() {
    ...
    try {
      ...
      if (imageFile) {
        const uploadedImage = await uploadPostImage(imageFile);
              ...
              >
                ...
                {p.imageUrl && (
                  <PostImage
                    ...
                    priority={index >= displayPosts.length - 8}
