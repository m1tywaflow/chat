...
export default function ChannelInfoModal({
  ...
  return (
    <>
      ...
      >
        <div
          className={`cim-modal w-[380px] max-h-[85vh] flex flex-col rounded-3xl ${GLASS_PANEL} overflow-hidden relative`}
          onClick={(e) => e.stopPropagation()}
        >
          <div className="pointer-events-none absolute inset-0 z-0 overflow-hidden">
          ...
          {view === "info" && (
            <>
              {/* header */}
              <div className="relative z-10 flex flex-col items-center gap-4 pt-8 pb-5 px-6 border-b border-white/[0.08]">
                <div
                  className="w-20 h-20 rounded-full overflow-hidden bg-[#1e2a3a] ring-2 ring-[#7c5cff]/25 flex items-center justify-center shrink-0 cim-avatar text-[#a893ff] text-2xl font-semibold"
                  onClick={() =>
                    channel.avatarUrl && setZoomUrl(channel.avatarUrl)
              ...
              {/* footer action */}
              <div className="relative z-10 flex-none border-t border-white/[0.08] p-3">
                {isOwner ? (
                  <button
                    onClick={onRequestDelete}
                    className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-red-500/10 hover:bg-red-500/[0.16] border border-red-500/20 text-[13px] font-semibold text-red-400 transition-colors cursor-pointer"
                  >
                    <Trash2 size={15} />
                    Delete channel
                  </button>
          ...
          {view === "settings" && (
            <>
              {/* header: settings mode */}
              <div className="relative z-10 flex flex-col items-center pt-8 pb-5 px-6 border-b border-white/[0.08]">
                <div
                  className="relative w-20 h-20 rounded-full overflow-hidden bg-[#1e2a3a] ring-2 ring-[#7c5cff]/25 flex items-center justify-center mb-4 cursor-pointer group"
                  onClick={() =>
                    document.getElementById("cim-avatar-input")?.click()
          ...
          {view === "subscribers" && (
            <>
              {/* subscribers header */}
              <div className="relative z-10 flex-none flex items-center gap-3 px-4 h-14 border-b border-white/[0.08]">
                <button
                  onClick={() => setView("info")}
                  className="w-8 h-8 flex items-center justify-center rounded-full text-zinc-400 hover:text-white hover:bg-white/[0.08] transition-colors cursor-pointer"
                >
                  <ChevronLeft size={18} />
                </button>
                <div className="text-[15px] font-semibold text-white">
                ...
                <button
                  onClick={onClose}
                  className="ml-auto w-8 h-8 flex items-center justify-center rounded-full text-zinc-400 hover:text-white hover:bg-white/[0.08] transition-colors cursor-pointer"
                >
                  <X size={16} />
                </button>
              </div>
                ...
                {!loadingSubs &&
                  subscribers.map((s) => {
                    ...
                    return (
                      <div
                        key={s.uid}
                        className="cim-row group flex items-center gap-3 px-3 py-2 rounded-2xl hover:bg-white/[0.05]"
                      >
                        <div
                          className="shrink-0 cim-avatar"
                          onClick={() => s.avatarUrl && setZoomUrl(s.avatarUrl)}
                        ...
                        {isOwner && !isThisOwner && (
                          <button
                            ...
                            disabled={removingUid === s.uid}
                            title="Remove subscriber"
                            className="shrink-0 opacity-0 group-hover:opacity-100 w-7 h-7 flex items-center justify-center rounded-full text-zinc-500 hover:text-red-400 hover:bg-red-500/[0.1] transition-all cursor-pointer disabled:opacity-40"
                          >
                            {removingUid === s.uid ? (
                              <Loader2 size={14} className="animate-spin" />
                            ) : (
